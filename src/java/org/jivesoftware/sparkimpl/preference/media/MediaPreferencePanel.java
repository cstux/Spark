...
public class MediaPreferencePanel  extends JPanel {
    ...
    private static Logger log = Logger.getLogger(MediaPreferencePanel.class);
    private Vector<CaptureDeviceInfo2> vectorAudioDevices;
    private Vector<CaptureDeviceInfo2> filterAudioDevices = new Vector<CaptureDeviceInfo2>(); 
    ...
    private JComboBox<String> audioDevice = new JComboBox<String>();
    private JComboBox<AudioSystem> audioSystem = new JComboBox<AudioSystem>();
    private JComboBox<String> playbackDevice = new JComboBox<String>();
    private JComboBox<String> videoDevice = new JComboBox<String>();
    ...
    public MediaPreferencePanel() {
        ...
        audioSystem.addActionListener (new ActionListener () {
...
//        
    ...
    {
        System.out.println(audioSystem.getSelectedIndex());
        AudioSystem system = vectorAudioSystem.get(audioSystem.getSelectedIndex());
        filterAudioDevices.removeAllElements();
        if (system == null) 
        {
            return;
        }
        ...
        for (CaptureDeviceInfo2 info : vectorAudioDevices )
        {
            String protocol = system.getLocatorProtocol();
            String audioLocator = info.getLocator().toString();
            if (audioLocator.startsWith(protocol))
            {
                filterAudioDevices.add(info);
            }                                   
        }
        for ( CaptureDeviceInfo infoCaptureDevice : filterAudioDevices)
        {                
    ...
    public void scanDevices()
    {
        ...
        // FMJ  
        System.setProperty(ConfigurationService.PNAME_SC_HOME_DIR_LOCATION, Spark.getSparkUserHome());
        System.setProperty(ConfigurationService.PNAME_SC_HOME_DIR_NAME, ".");
        System.setProperty(ConfigurationService.PNAME_SC_CACHE_DIR_LOCATION, Spark.getSparkUserHome());
        System.setProperty(ConfigurationService.PNAME_SC_LOG_DIR_LOCATION, Spark.getSparkUserHome());
        ...
        // read video devices
        vectorVideoDevices = CaptureDeviceManager.getDeviceList(new VideoFormat(VideoFormat.H263_RTP));
...
//      
    ...
    public String getAudioDevice() {
        if (audioDevice.getSelectedIndex() >= 0) {
            return filterAudioDevices.get(audioDevice.getSelectedIndex()).getLocator().toExternalForm();
        }
        return "";
    }
